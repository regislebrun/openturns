...
Point Mesh::computeSimplicesVolume() const
{
  ...
  // The mesh is of codimension 1
      ...
      else
	{
	  ...
	  for (UnsignedInteger index = 0; index < nrSimplices; ++index)
	    {
	      // We know (https://urresearch.rochester.edu/fileDownloadForInstitutionalItem.action?itemId=13451&itemFileId=31154) that the hyperplane goes through the center of the vertices (ie the mean) and has the eigenvector associated with the smallest eigenvalue of M'M (a semi-definite symmetric real matrix, so its eigenvalues are all nonnegative)
