...
int main(int, char *[])
{
  ...
  try
  {
    ...
    // The invalid inputs must be rejected
    {
      ...
      // A simplex referring to an unknown vertex must be rejected
      {
        Indices badIndices(indices);
        badIndices[2] = 5;
        caught = false;
        try
        {
      ...
      // A simplex with the wrong number of vertices must be rejected
      {
        Indices badIndices(2);
        badIndices[0] = 0;
        badIndices[1] = 1;
        caught = false;
        try
