...
rangePDF = ot.Interval(-pi/2, pi/2)
...
exact_PDF = ot.LinearCombinationFunction([unscaled_PDF], [1/norm_fact])
...
ks_algo.setLowerBound(-pi/2)
ks_algo.setUpperBound(pi/2)
ks_pdf = ks_algo.build(sample)

g = ks_pdf.drawPDF()
draw = exact_PDF.draw(-pi/2, pi/2, 1001).getDrawable(0)
draw.setLineWidth(2)
g.add(draw)
