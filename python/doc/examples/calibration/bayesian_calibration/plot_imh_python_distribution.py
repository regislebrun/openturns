...
graph.setTitle(r"Christian Robert function: $f(x) =  0.5(2 + sin^2 x) e^{ -x( 2 + cos^33x + sin^3 2x)}, \quad x \in [0, 2\pi]$")
graph.setXTitle("x")
...
class NewDistribution(ot.PythonDistribution):
    ...
    def computeCDF(self, X):
        ...
        if X[0] >= self.rangeUp_:
            return 1.0
        return ot.GaussLegendre([64]).integrate(self.PDF_, ot.Interval(self.rangeLow_, X[0]))[0]
...
g.add(draw_RoU)
g.setLegends(['RoU size = ' + str(size), 'exact PDF', 'RoU (size = ' + str(size2) + ')'])
