...
#
exact_PDF = ot.LinearCombinationFunction([f], [1/norm_fact])
...
t0 = time()
sample_RoU = newDist_RoU.getSample(size)
t1 = time()
sample_generic = newDist_generic.getSample(size)
t2 = time()

print('Ratio of Uniforms speed = ', size / (t1-t0), 'rng/s')
print('Generic algorithm speed = ', size / (t2-t1), 'rng/s')
...
ks_algo.setUpperBound(2*pi)
ks_pdf = ks_algo.build(sample)

g = ks_pdf.drawPDF()
draw = exact_PDF.draw(0.0, 2*pi, 1001).getDrawable(0)
draw.setLineWidth(2)
g.add(draw)
