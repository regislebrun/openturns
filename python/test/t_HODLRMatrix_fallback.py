...
class BlockAssemblyFunction:
    ...
    Every entry of the off-diagonal blocks is below the hard-coded pivot
    threshold (1.0e-14) of the ACA in HODLRNode::lowRankApprox, so all rows
    are rejected and the rank-zero fallback branch (index.empty()) is taken.
    """
...
def make_params(leaf, max_rank, factorization):
...
# === Test 1: single-level tree, fallback block with rank capped at maxRank ===
# The top-level off-diagonal block is 10x10 and falls back to a deterministic
# rank-2 approximation (nCols > maxRank). Regression case for the fallback.
print("=== Test 1: single-level fallback with capped rank (LU) ===")
n1 = 20
vertices1 = ot.Sample([[i * 0.1] for i in range(n1)])
af1 = BlockAssemblyFunction(vertices1, n1 // 2, 0.2)
params1 = make_params(leaf=n1 // 2, max_rank=2, factorization='LU')
...
# The fallback must store maxRank columns only: the 10x10 degenerate block
# contributes 2 * 10 * 2 = 40 entries on top of the two 10x10 dense leaves.
cr1 = hodlr1.compressionRatio()
ott.assert_almost_equal(ot.Point([cr1[0]]), ot.Point([240]))
...
params3 = make_params(leaf=n3 // 2, max_rank=2, factorization='LLt')
...
print("\n=== Test 4: unbounded maxRank fallback (LU) ===")
n4 = 10
vertices4 = ot.Sample([[i * 0.1] for i in range(n4)])
af4 = BlockAssemblyFunction(vertices4, n4 // 2, 0.2)
params4 = make_params(leaf=n4 // 2, max_rank=0, factorization='LU')
...
params5 = make_params(leaf=4, max_rank=0, factorization='LLt')
...
# reachable for the non-Cholesky method.
print("\n=== Test 6: singular block raises in LU factorization ===")
n6 = 8
...
params6 = make_params(leaf=2, max_rank=2, factorization='LU')
...
params7 = make_params(leaf=4, max_rank=2, factorization='LU')
...
hodlr10.factorize('LU')
print("  PASS")

# === Test 8: copies drop the tree but must report their true condition ===
# Copy operations drop the HODLR tree. They must reset the derived state
# (isFactorized_, isCholesky_, logDet_, shiftAccumulated_) so the copy reports
# "not assembled" instead of silently returning stale factorization results.
...
params8 = make_params(leaf=4, max_rank=2, factorization='LU')
...
params9 = make_params(leaf=4, max_rank=2, factorization='LU')
